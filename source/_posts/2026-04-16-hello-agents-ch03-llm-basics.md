...
- LLM
- hello-agents
description: "一句话结论：GPT不理解语言——它是一个极其精密的概率机器，通过预测下一个词是什么来完成几乎一切语言任务。但正是这个不理解，让它比任何理解的系统都更强大。"
---
...
本章解释 LLM 原理（统计预测、Tokenizer、Transformer 注意力），对比维度：LLM 内部机制 vs 相邻章节的 Agent 概念与范式 vs 真实生产中的模型服务框架。

## 对比分析

### 一、本章概念 vs 相邻章节

| 维度 | ch03 LLM 原理 | ch01 智能体是什么 | ch02 历史 | ch04 经典范式 |
|------|---------------|-------------------|----------|--------------|
| 抽象层级 | 模型层（统计/数学） | 系统层（PRAM） | 时代层（演进） | 行为层（范式） |
| 是否含公式 | 含（注意力示意） | 不含 | 不含 | 不含 |
| 与本章互补 | —— | 解释"为什么 LLM 是大脑" | 解释"为什么是这一波" | 解释"怎么用大脑" |

### 二、对比生产中类似框架

| 角色 | 服务 / 框架 | 与本章内容的对应 |
|------|--------------|-------------------|
| 模型 API | OpenAI、Anthropic、Gemini | 本章描述的能力直接来自它们 |
| 开源权重 | Llama、Qwen、Mistral | 可本地部署，体验同样的"文字接龙" |
| 推理引擎 | vLLM、TGI、SGLang | 把 Transformer 注意力高效跑起来 |
| Agent SDK | LangChain、LlamaIndex | 把 LLM 包装成 Agent 工具 |

### 三、本章观点的优缺点

优点：
- 把"不理解语言却能用语言"这件事讲透
- Tokenizer / 上下文窗口这两个工程关键点都点到

缺点：
- 没有讲 RLHF / 指令微调（这部分留给 ch11）
- 对推理成本、KV Cache 等工程权衡涉及少

### 四、何时选

- 想理解"为什么 Prompt 工程有用"：读 ch03
- 想直接做应用：可只扫一遍"上下文窗口"小节，再进入 ch04+

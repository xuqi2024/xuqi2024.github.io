...
ASPICE 是汽车软件过程评估的事实标准，与 ISO 26262（功能安全）、CMMI（通用过程改进）在目标和定位上既有重叠也有互补。

### 维度对比表

| 维度 | Automotive SPICE | ISO 26262 | CMMI |
|------|------------------|-----------|------|
| 关注点 | 软件开发过程能力 | 功能安全（电气/电子系统） | 通用组织过程能力 |
| 范围 | 汽车软件全生命周期（系统工程+软件工程） | 完整安全生命周期（概念→生产→报废） | 跨行业的组织级过程 |
| 等级模型 | 6 级（0-5），关注过程制度化 | ASIL A-D（4 级），关注风险降低 | 5 级（1-5），关注组织成熟度 |
| 评估方式 | intacs 认证 assessor | 认证机构审计 | SCAMPI 评估 |
| 强制性 | OEM 普遍强制（尤其等级 3+） | 法规要求（安全相关 ECU） | 自愿性 |
| 与 AI/Agent 关联 | 提供过程框架基础 | 不直接涉及 AI 行为安全 | 间接提供组织治理基础 |

### 优缺点

**Automotive SPICE**
- 优点：行业内普遍认可；评估模型明确（过程维度+能力等级）；与 ISO 26262 互补；提供具体的过程参考模型与工作产品。
- 缺点：文档重、流程繁琐；对小团队/初创公司负担较重；不直接覆盖 AI/ML 行为安全。

**ISO 26262**
- 优点：覆盖完整安全生命周期；ASIL 分级明确；强制性强（法规层面）；与硬件/系统/软件全栈协同。
- 缺点：专注于功能安全，不涵盖非安全相关过程；AI/ML 行为安全的覆盖有限（需结合 ISO/PAS 21448 SOTIF）。

**CMMI**
- 优点：通用性强、跨行业适用；组织级视角更全面；可与 ASPICE 互补。
- 缺点：不针对汽车行业具体过程；评估周期长；汽车 OEM 不直接采信。

### 何时选

- **选 ASPICE**：你的产品是汽车软件组件，需要进入 OEM 供应链，是 Tier-1/Tier-2 的硬性门槛。
- **配合 ISO 26262**：当系统涉及功能安全（如制动、转向、动力域），ASPICE 管过程，26262 管安全。
- **结合 CMMI**：当组织希望整体过程能力提升（多项目、多团队）时，可与 ASPICE 形成"组织级 + 产品级"双层治理。

### 参考资料

- intacs 官方：https://www.intacs.info/
- Automotive SPICE PAM 4.0（官方过程参考模型）
- ISO 26262-6 软件产品安全

...
- C++
- C++17
description: "C++14 写一个变参 print 函数需要几步？答案是：至少 50 行递归模板代码。C++17 的折叠表达式让这件事变成一行。"
---
...
本章讲 C++17 的 Fold Expressions（折叠表达式），对比维度：本特性 vs C++11/14 旧写法 vs 其他语言变参处理。

## 对比分析

### 一、本特性 vs 旧写法

| 维度 | C++17 fold 写法 | C++14 递归展开 | 影响 |
|------|------------------|----------------|------|
| 变参打印 | `(std::cout << ... << args)` | 写一个 base case + 递归 | 一行替代 30+ 行 |
| 变参求和 | `(args + ... + 0)` | 递归 + 特化 | 一行搞定 |
| 变参布尔与 | `(args && ...)` | 同样递归 | 一行搞定 |
| 编译期处理 | 可与 `if constexpr` 组合 | 模板技巧复杂 | 表达力强 |

### 二、对比其他语言

| 语言 | 变参处理 | 是否支持 fold | 备注 |
|------|----------|---------------|------|
| C++17 | ✅ 一元 / 二元 fold | ✅ | 编译期可推导 |
| Java | 可变参数 `T...` | ❌ | 无 fold 语义 |
| Python | `*args` | ✅ 通过 `functools.reduce` | 运行时 |
| Rust | 宏 + 迭代器 | ✅ 通过宏变参 | 比 fold 更"宏" |
| Haskell | 类型类 foldr/foldl | ✅ 语言级 | 类型系统支撑最强 |

### 三、优缺点

优点：
- 大幅缩短变参模板代码
- 与 `if constexpr` 配合可写出简洁 SFINAE-free 代码

缺点：
- 一元 fold 在空包时需要显式 identity（`+ 0`、`&& true` 等）
- 调试栈不如递归直观

### 四、何时选

- 任何"对所有参数做同一操作"的场景：首选 fold
- 需要复杂逻辑（如先转换再聚合）：回到递归或 fold + lambda

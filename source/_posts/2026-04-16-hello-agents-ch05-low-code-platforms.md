...
- Agent框架
- hello-agents
description: "低代码平台不是玩具，而是让80%的Agent需求在不写一行代码的情况下落地的生产力工具——剩下20%的复杂需求，才是真正需要开发框架的地方。"
---
...
本章对比 Coze、Dify、FastGPT 等低代码 Agent 平台，对比维度：低代码平台 vs 相邻章节的代码框架 vs 国内外类似 SaaS 产品。

## 对比分析

### 一、本章概念 vs 相邻章节

| 维度 | ch05 低代码平台 | ch04 思考范式 | ch06 框架实战 | ch07 自研框架 |
|------|----------------|---------------|--------------|--------------|
| 用户 | 非程序员 / 产品 / 运营 | 概念层 | 开发者 | 进阶开发者 |
| 介入深度 | 配置 + 拖拽 | 范式选择 | API 调用 | 全部自实现 |
| 学习曲线 | 低 | 中 | 中 | 高 |
| 与本章互补 | —— | 决定"平台内可用哪些范式" | 决定"何时离开平台" | 决定"如何自己造平台" |

### 二、对比生产中类似平台

| 平台 | 主要特点 | 与本章对比的差异点 |
|------|----------|---------------------|
| Coze（字节） | 插件生态丰富、国内模型接入顺 | 更偏 C 端 / 模板化 |
| Dify | 开源 + 可私有化、Workflow 编排强 | 偏企业落地 |
| FastGPT | 知识库 + RAG 见长 | 偏检索增强类应用 |
| n8n / Flowise | 通用工作流 | 通用自动化，不只 Agent |

### 三、本章观点的优缺点

优点：
- 用同一业务需求横向比较多个平台，便于决策
- 点出"低代码不是终态"的现实

缺点：
- 平台迭代快，截图 / 价格信息容易过时
- 没有给出"低代码 vs 代码"的迁移路径细节

### 四、何时选

- 验证想法 / 做 MVP：低代码平台
- 要深入定制 / 数据合规：进入 ch06 看代码框架
- 平台能力不够：进入 ch07 自己造

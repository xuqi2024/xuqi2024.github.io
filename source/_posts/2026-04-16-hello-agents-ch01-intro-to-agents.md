...
本章围绕"什么是 Agent"展开，对比维度为：Agent 概念本身 vs 相邻章节的 LLM 基础与经典范式 vs 生产中常见的多 Agent 框架。

## 对比分析

### 一、本章概念 vs 相邻章节

| 维度 | ch01 智能体是什么 | ch02 智能体60年 | ch03 LLM 原理 | ch04 思考三剑客 |
|------|--------------------|----------------|---------------|----------------|
| 关注点 | Agent 与 LLM 的边界定义 | Agent 的历史演进 | 模型本身的统计原理 | Agent 思考的具体范式 |
| 抽象层级 | 概念与组成（PRAM） | 哲学与时代背景 | 模型内部机制 | 任务执行模板 |
| 是否含代码 | 仅架构示意 | 无 | 含数学示意 | 含代码片段 |
| 何时需要 | 入门第一站 | 想了解为何现在火 | 想理解能力上限 | 想落地 Agent 逻辑 |

### 二、对比生产中类似框架

| 框架 / 项目 | 与本章 Agent 概念的对应关系 | 强调的能力 | 适用场景 |
|-------------|------------------------------|------------|----------|
| LangChain | 提供 LLM + Tools + Chain 的胶水层，对应 PRAM 中的 R/A | 工具调用、RAG | 快速搭建单 Agent |
| AutoGen | 多 Agent 对话范式，对应"多个 Agent 协作" | 多角色协作 | 研究型、多视角任务 |
| CrewAI | 角色+任务+流程，对应 ch04 中 Plan-and-Execute | 团队协作 | 业务流程自动化 |
| smolagents | 极简代码 Agent，对应 ch07 手写框架 | 透明可控 | 教学与定制 |

### 三、本章观点的优缺点

优点：
- 用 PRAM 框架把"感知-推理-行动-记忆"具象化，初学者容易建立心智模型
- 表格形式明确划出 LLM 与 Agent 的能力边界
- 工作循环图覆盖了"何时结束任务"的关键判断

缺点：
- 偏概念化，没有涉及具体工具调用协议（如 MCP，将在 ch10 展开）
- 对记忆模块只点到为止，详细机制要等到 ch08

### 四、何时选

- 如果你完全没接触过 Agent：从 ch01 开始，再按顺序往下
- 如果只想了解概念：读完 ch01 + ch04 即可建立完整心智图
- 如果想立即动手：跳过 ch02 历史，先看 ch05 / ch06 / ch07

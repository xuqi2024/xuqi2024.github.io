...
本章讲 C++20 的 `<=>` Spaceship Operator（三路比较），对比维度：本特性 vs C++17 旧写法 vs 其他语言比较运算符。
...
|------|------------|-------------|------|
| 自动生成 <,>,<=,>=,==,!= | `auto operator<=>(const T&) const = default;` | 手写 6 个运算符 | 一行替代 6 个 |
| 比较结果类型 | `std::strong_ordering / weak_ordering / partial_ordering` | bool | 区分强弱序 |
| 与 C++20 默认比较联动 | 自动重写 ==, != | 不支持 | 一致性 |
...
|------|-----------------|------|
| C++20 | `<=>` + 默认比较 | 显式区分强弱 |
| Java | `Comparable.compareTo` | 二路 |
| Python | `__lt__/__eq__` 全套 | 六路自己写 |
| Rust | `PartialOrd / Ord` trait | 类型化结果 |
| Haskell | `Ord` 类型类 | 类型化 |
...
缺点：
- 与旧代码混用时仍需注意 `<` / `==` 的一致性
- C++20 之前的老 ABI 兼容性需评估

### 四、何时选

- 新代码、值类型：直接 `<=> = default`
- 涉及浮点：注意 `partial_ordering`

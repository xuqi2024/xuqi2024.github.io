...
本章用 LangChain、AutoGen、CrewAI 三个框架实战多 Agent，对比维度：三大框架特性 vs 相邻章节的低代码平台与自研框架 vs 工业界同类多 Agent 框架。

## 对比分析

### 一、本章概念 vs 相邻章节

| 维度 | ch06 三大框架 | ch05 低代码平台 | ch04 经典范式 | ch07 自研框架 |
|------|----------------|----------------|--------------|--------------|
| 控制粒度 | 代码 | 拖拽 | 范式 | 全部自实现 |
| 适合人群 | 应用开发者 | 非开发者 | 学习者 | 框架作者 |
| 多 Agent 支持 | 强 | 部分 | 单 Agent | 可定制 |
| 与本章互补 | —— | 上手快但难定制 | 给出思维模板 | 理解底层 |

### 二、对比生产中类似框架

| 框架 | 多 Agent 模型 | 风格 | 与本章三者对应 |
|------|---------------|------|----------------|
| LangChain | Agent + Tools 编排 | 通用胶水 | 对应 LangChain 部分 |
| AutoGen | 对话驱动多 Agent | 研究派 | 对应 AutoGen 部分 |
| CrewAI | 角色 + 任务 + 流程 | 业务派 | 对应 CrewAI 部分 |
| smolagents | 极简 code Agent | 教学派 | ch07 风格 |
| LlamaIndex Workflows | 事件驱动流水线 | 数据派 | 与 LangGraph 相近 |
...
缺点：
- 框架 API 迭代快，代码片段需要看对应版本
- 没有覆盖 LangGraph 这一当前主流的状态化方案（建议看 ch10 后的衍生阅读）

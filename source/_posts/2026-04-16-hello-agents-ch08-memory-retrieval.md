...
本章讲 RAG 与记忆系统，对比维度：记忆子系统 vs 相邻章节的自研框架与 Context Engineering vs 业界 Mem0、Letta、Cognee 等专门记忆层。

## 对比分析

### 一、本章概念 vs 相邻章节

| 维度 | ch08 记忆与检索 | ch07 自研框架 | ch09 Context Engineering | ch10 协议 |
|------|------------------|---------------|--------------------------|----------|
| 抽象层级 | 记忆子系统 | Runtime | 上下文调度 | 对外交互 |
| 解决问题 | "Agent 失忆" | "如何跑通循环" | "Prompt 该放什么" | "Agent 之间怎么对话" |
| 与本章互补 | —— | 提供插入点 | 提供调度策略 | 提供外部知识接入 |

### 二、对比生产中类似项目

| 项目 | 关注点 | 与本章对比 |
|------|--------|------------|
| LangChain Memory | 内置多种 Memory 类 | 与 LangChain 紧耦合 |
| LlamaIndex | RAG / 索引见长 | 数据接入能力强 |
| Mem0 | 通用记忆层 | 跨会话、跨应用 |
| Letta | 带记忆的 Agent 运行时 | 把记忆做成产品核心 |
| Cognee | 知识图谱 + RAG | 强调结构化推理 |
...
- 一般对话：短期 + 长期记忆 + 向量检索足够
- 需要"记住用户偏好跨应用"：用 Mem0 类方案
- 需要"语义关系推理"：考虑 Cognee / Graphiti

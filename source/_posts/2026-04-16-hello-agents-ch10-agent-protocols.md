...
本章讲解 MCP、A2A、ANP 等 Agent 协议，对比维度：协议层 vs 相邻章节的框架与 Context Engineering vs 业界实际协议生态。

## 对比分析

### 一、本章概念 vs 相邻章节

| 维度 | ch10 Agent 协议 | ch06 框架实战 | ch07 自研框架 | ch09 Context Engineering |
|------|------------------|--------------|----------------|--------------------------|
| 抽象层级 | 对外协议 | 编程接口 | Runtime | 内部编排 |
| 解决的问题 | Agent / 工具 / 跨厂商互通 | 怎么用现成框架 | 怎么跑起来 | 怎么填上下文 |
| 与本章互补 | —— | 协议通过框架实现 | 协议可由自研框架接入 | 协议承载上下文结构 |

### 二、对比生产中类似协议

| 协议 | 出品方 | 关注点 | 与本章对应 |
|------|--------|--------|------------|
| MCP | Anthropic | 工具 / 资源 / Prompt 标准化 | 本章核心 |
| A2A | Google / 社区 | Agent 间通信 | 本章核心 |
| ANP | 社区 | 去中心化 Agent 网络 | 本章核心 |
| OpenAI Function Calling | OpenAI | 工具调用事实标准 | 协议前身 |
| AG-UI | 社区 | Agent ↔ 前端交互 | ch10 之外的延展 |
...
缺点：
- 协议仍在快速演进，建议以"概念"读，而非"具体字段"
- 安全 / 鉴权部分较少

### 四、何时选

- 单 Agent + 几个工具：先用 Function Calling / MCP 即可
- 多 Agent 协作：A2A
- 跨组织 / 跨厂商：ANP

...
- 工具调用
- hello-agents
description: "核心结论：多Agent协作不是让一个LLM变得更聪明，而是让多个专职LLM分工合作——就像雇了一支旅行规划团队，每个人只干自己最擅长的事。"
series: hello-agents
...
本章用 Agent 规划日本 5 日游实战，对比维度：本案例实现 vs 相邻章节的范式与框架 vs 业界旅行 / 行程类 Agent 产品。

## 对比分析

### 一、本章概念 vs 相邻章节

| 维度 | ch13 旅行助手 | ch04 经典范式 | ch06 三大框架 | ch09 Context Engineering |
|------|--------------|---------------|--------------|--------------------------|
| 抽象层级 | 应用案例 | 范式 | 实现工具 | 上下文策略 |
| 关注问题 | "端到端怎么搭" | "Agent 怎么想" | "怎么用框架" | "Prompt 怎么写" |
| 与本章互补 | —— | 本章是 ReAct + Plan 范式的实例 | 本章基于具体框架 | 本章是上下文优化的范例 |

### 二、对比生产中类似产品

| 产品 | 关注点 | 与本章对比 |
|------|--------|------------|
| 携程 / 去哪儿 AskBot | 真实订单闭环 | 数据接入更全，但流程固定 |
| Layla / Roam Around | 多日行程 | 行程生成 + 地图联动 |
| Google Gemini 行程 | 集成 Gmail / Search | 信息源更深 |
| 自研 MCP + LLM | 可定制 | 与本章路径一致 |
...
缺点：
- 没有真实接入 OTA 接口，停留在工具调用模拟
- 没有讨论失败重试 / 用户确认

### 四、何时选

- 学习：跟做本章
- 做产品：选 OTA / MCP 接入，并加 ch12 评测

...
- 开源项目
- hello-agents
description: "没有严格的评估，再好的 Agent 都只是一个感觉良好的黑盒——评估是将工程直觉转化为可量化结论的唯一路径。"
---
...
本章讲 Agent 评估体系，对比维度：评测方法 vs 相邻章节的训练与工程章节 vs 业界 Agent 评测基准与平台。

## 对比分析

### 一、本章概念 vs 相邻章节

| 维度 | ch12 评估 | ch11 Agentic RL | ch13 旅行助手 | ch14 深度研究 |
|------|-----------|----------------|--------------|--------------|
| 抽象层级 | 评测方法 | 训练方法 | 应用样例 | 应用样例 |
| 关注问题 | "怎么知道好不好" | "怎么让它变好" | "做出来是什么样" | "做出来是什么样" |
| 与本章互补 | —— | 评测可作训练 reward | 评测对象 | 评测对象 |

### 二、对比生产中类似评测框架

| 框架 / 基准 | 关注点 | 与本章对应 |
|-------------|--------|------------|
| Harbor | Agent 评测平台 | 本章工程化代表 |
| AgentBench | 综合能力 | 本章方法论 |
| SWE-bench | 代码 Agent | 应用类 |
| GAIA | 多步推理 | 应用类 |
| HotpotQA | 多跳问答 | RAG 类 |
...
缺点：
- 没有覆盖"成本"维度（$/任务）
- 人类评测与 LLM-as-judge 的偏差讨论较少

### 四、何时选

- 内部 MVP：人工抽样 + LLM-as-judge
- 对外发布 / 上线：建立结构化评测集 + A/B
- 学术对比：参考 AgentBench / SWE-bench 风格

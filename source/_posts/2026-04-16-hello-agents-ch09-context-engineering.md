...
本章讲 Context Engineering（上下文工程），对比维度：上下文工程 vs 相邻章节的记忆系统与协议 vs 业界 Prompt 工程化与"长上下文"相关实践。

## 对比分析

### 一、本章概念 vs 相邻章节

| 维度 | ch09 Context Engineering | ch08 记忆检索 | ch10 协议 | ch04 经典范式 |
|------|--------------------------|--------------|----------|---------------|
| 抽象层级 | Prompt / 上下文编排 | 数据来源 | 对外接口 | 思考模板 |
| 关注问题 | "该把什么喂给 LLM" | "数据怎么存 / 怎么取" | "Agent 之间怎么通信" | "Agent 怎么思考" |
| 与本章互补 | —— | 提供材料 | 提供工具描述 | 提供结构 |

### 二、对比生产中类似实践

| 实践 / 工具 | 关注点 | 与本章对比 |
|-------------|--------|------------|
| Prompt 模板化（DSPy） | 自动优化 Prompt | 把 Context Engineering 自动化 |
| 长上下文模型（Gemini 1.5 / Claude） | 200k+ 上下文 | 减少主动检索需求 |
| RAG + 压缩（LLMLingua） | 节省 token | 上下文过载时的兜底 |
| 结构化输出（Instructor / JSON Schema） | 强制结构 | 让 LLM 输出更可控 |

### 三、本章观点的优缺点

优点：
- 把"上下文"从 Prompt 升级成系统性问题
- 给出"窗口有限"下的取舍思路

缺点：
- 与 ch08 部分概念有重叠，需要自行区分"记忆"与"上下文"
- 没有给出"如何自动评测 Prompt"的工程方案

### 四、何时选

- 单任务、固定模板：普通 Prompt 工程即可
- 多任务、跨模型：用 DSPy 类自动优化
- 超长文档：考虑长上下文模型 + 摘要，而非纯检索
